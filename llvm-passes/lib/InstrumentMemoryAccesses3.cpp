...
void setUpFunctions(Module &M, IRBuilder<> *Builder) {
    ...
    llvm::Constant *FileLocationStr =
        llvm::ConstantDataArray::getString(CTX, getModuleIdentifier + ".txt");
    ...
    for (auto &F : M) {
        for (auto &BB : F) {
            for (auto &Inst : BB) {
                ...
                // get the identifier of the current loop?
                auto loopIdentifier = getLoopFor(&BB);
                ...
                if (store == nullptr) {
                    Builder.CreateCall(Fprintf,
                                       {filePtr, LoadResultFormatStrPtr,
                                        identifier, memoryAccessLocation, loopIdentifier});
                } else {
                    Builder.CreateCall(
                        Fprintf, {filePtr, StoreResultFormatStrPtr, identifier,
                                  memoryAccessInst->getOperand(1), loopIdentifier});
                }
